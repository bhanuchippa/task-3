...
IMAGENET_TO_CIFAR10_MAP = {
    ...
    "Horse": [
        'horse', 'colt', 'foal', 'stallion', 'mare', 'zebra', 'pony', 'donkey', 'mule', 'wild_ass'
    ],
    "Airplane": [
...
# Get all ImageNet labels by decoding dummy predictions
import numpy as np
dummy_preds = np.zeros((1, 1000))
# Let's decode all classes by setting each to 1.0 sequentially or accessing the decoding map directly
# We can load the json map directly from keras or decode a diagonal matrix.
# A diagonal matrix of size 1000x1000 will contain 1.0 for each class.
diag = np.eye(1000)
...
for cls, lst in mapped_counts.items():
    ...
    print(f"  Examples: {lst[:10]}")

print(f"Unmapped: {len(unmapped_classes)} classes")
print(f"Examples of unmapped: {unmapped_classes[:20]}")
